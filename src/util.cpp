...
static std::vector<unsigned char> heightToNormal(const unsigned char* rgba, int w, int h, float bumpiness, bool smooth)
{
    ...
    auto clampX = [&](int x) { return std::min(std::max(x, 0), w - 1); };
    auto clampY = [&](int y) { return std::min(std::max(y, 0), h - 1); };
    ...
    if (smooth)
    {
        ...
        for (int y = 0; y < h; ++y)
            ...
            {
                float s = 0.0f;
                for (int k = -R; k <= R; ++k) s += hf[(size_t)y * w + clampX(x + k)];
        ...
        for (int y = 0; y < h; ++y)
            ...
            {
                float s = 0.0f;
                for (int k = -R; k <= R; ++k) s += tmp[(size_t)clampY(y + k) * w + x];
                hf[(size_t)y * w + x] = s / (2 * R + 1);
            }
    }

    auto at = [&](int x, int y) -> float { return hf[(size_t)clampY(y) * w + clampX(x)]; };
...
bool convertImageToDDS(const fs::path& inputPath, const fs::path& outputPath, const ImageImportOptions& opt)
{
    ...
    if (opt.isNormalMap)
    {
        ...
        for (size_t i = 0; i < (size_t)srcW * srcH; ++i) src[i * 4 + 3] = 255;
    }
    else
    {
        // --- Alpha source ------------------------------------------------
    ...
    {
        int lw = w, lh = h;
        while (lw > 1 || lh > 1) { lw = mipHalf(lw); lh = mipHalf(lh); ++levels; }
    }
    ...
    for (int level = 0; level < levels; ++level)
    {
        if (level > 0)
        {
            ...
            if (!ok) { std::cerr << "Mip resize failed: " << inputPath << "\n"; return false; }
            prev.swap(next);
            lw = nw; lh = nh;
        }
    ...
    if (levels > 1) flags |= 0x20000;          // DDSD_MIPMAPCOUNT
    ...
    if (levels > 1) header.caps.dwCaps1 |= 0x8 | 0x400000; // COMPLEX | MIPMAP
...
static kString svPrettyLabel(const kString& name)
{
    kString n = name;
    size_t dot = n.rfind('.');
    if (dot != kString::npos) n = n.substr(dot + 1);
...
std::vector<ShaderVar> parseShaderVars(const kString& source)
{
    ...
    while (std::getline(ss, line))
    {
        ...
        ShaderVar v;
        if (!(ls >> v.type)) continue;
        if (!(ls >> v.name)) continue;
...
{
    if (shaderName == "Unlit") return "SHADER_MESH_FLAT";
    if (shaderName == "Phong") return "SHADER_MESH_PHONG";
    if (shaderName == "PBR")   return "SHADER_MESH_PBR";
    return "";
}
